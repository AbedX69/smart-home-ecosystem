...
#define GC9A01_NORON        0x13    // Normal (full) display mode
 ...
 * =============================================================================
 * 
 * Partial mode only refreshes a horizontal strip of the display.
 * The rest of the display holds its content but stops refreshing.
 * 
 * USE CASES:
 *     - Status bar that updates frequently (clock, battery, etc.)
 *     - Scrolling text ticker
 *     - Power saving when only part of screen needs updating
 * 
 * HOW IT WORKS:
 *     
 *     Normal mode:          Partial mode (rows 100-140):
 *     ┌─────────────┐       ┌─────────────┐
 *     │ refreshing  │       │   frozen    │
 *     │ refreshing  │       │   frozen    │
 *     │ refreshing  │       ├─────────────┤
 *     │ refreshing  │  -->  │ REFRESHING  │ ← only this strip updates
 *     │ refreshing  │       ├─────────────┤
 *     │ refreshing  │       │   frozen    │
 *     └─────────────┘       └─────────────┘
 *     
 *     Power: ~25mA          Power: ~15mA (depending on strip size)
 * 
 * EXAMPLE:
 *     
 *     // Show a status bar at bottom, freeze rest
 *     display.setPartialArea(200, 239);
 *     
 *     // Update only the status bar
 *     display.fillRect(0, 200, 240, 40, COLOR_BLACK);
 *     display.drawString(10, 210, "12:34", COLOR_WHITE);
 *     
 *     // Back to normal full-screen updates
 *     display.setNormalMode();
 * 
 * NOTE:
 *     - Frozen areas retain their content but won't update if you draw to them
 *     - Round display: partial area is still rectangular (full width)
 *     - Call setNormalMode() before updating frozen areas
 */

void GC9A01::setPartialArea(uint16_t startRow, uint16_t endRow) {
    ...
    // Set partial area (start and end rows)
    sendCommand(GC9A01_PTLAR);
    sendData16(startRow);
    sendData16(endRow);
    
    // Enable partial mode
    sendCommand(GC9A01_PTLON);
    partialMode = true;
...
void GC9A01::setNormalMode() {
    sendCommand(GC9A01_NORON);
    partialMode = false;
